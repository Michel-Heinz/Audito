...
program example
    ...
    iniGrad = 0._r8
    ePot = FGL_t(0._r8, iniGrad, 0._r8)
...
contains
    ...
    subroutine gradDescent(coords, ePot, stepSize, maxIter, maxLss, verbose)
        type(FGL_t), intent(inout) :: coords(:), ePot
    ...
    subroutine getIniCoords(coords, seed)
        type(FGL_t), intent(inout) :: coords(:)
        ...
        !because the derivative of 'x' is 1. The second derivative of 'x' is 0. Thus, the Laplacian is initialized
        !as 0.
        !Example for the x and y coordinate of atom 1 of x atoms:
        !
        !coords(1) = FGL_t(2.48_r8, [1,0, 'filled with zeros' ...], 0)         ^
        !                            |
        !coords(2) = FGL_t(3.94_r8, [0,1, 'filled with zeros' ...], 0)           ^
        !                              |
        ...
        do i = 1, SIZE(coords)
            ...
            iniGrad(i) = 1._r8
            coords(i) = FGL_t(coordsTemp(i), iniGrad, 0._r8)
            iniGrad = 0._r8
        end do
    ...
    !and the Laplacian is also calculated!
    pure subroutine calcPotE(coords, ePot)
        implicit none
        type(FGL_t), intent(in) :: coords(:)
        type(FGL_t), intent(inout) :: ePot
